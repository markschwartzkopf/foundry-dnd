...
const npcAddModal = document.getElementById('npc-adder') as HTMLDivElement;
...
const npcAddWindow = document.getElementById(
  'npc-add-window'
) as HTMLDivElement;
...
const addMonster = document.getElementById('add') as HTMLDivElement;
const addNPC = document.getElementById('add-npc') as HTMLDivElement;
...
const npcIdInput = document.getElementById('npc-id-input') as HTMLInputElement;
...
addNPC.onclick = () => {
  npcAddModal.style.display = 'flex';
  npcIdInput.focus();
};

npcIdInput.onkeydown = (ev) => {
  if (
  ...
  if (ev.key === 'Enter') {
    nodecg
      .sendMessage('getNPCdata', npcIdInput.value)
      .then((data: Character) => {
    ...
    npcIdInput.value = '';
    npcAddModal.style.display = 'none';
  }
};
...
function playerDiv(
  ...
  let info = '';
  if (player.type === 'pc' || player.type === 'npc') {
    b.innerHTML =
  ...
  if (kind === 'combat' || player.type === 'monster' || player.type === 'npc') {
  ...
  if (player.type === 'monster' || player.type === 'npc') {
    ...
    lookDiv.onclick = () => {
      if (player.type === 'monster') {
        ...
        window.open(
          `https://www.dndbeyond.com/characters/${player.character.id}`,
          '_blank'
        );
      }
    };
...
function modifyPlayer(
  ...
  initInput.oninput = () => {
    ...
    if (!isNaN(val)) {
      ...
      } else if (player.type === 'monster' || player.type === 'npc') {
        NodeCG.waitForReplicants(battleNPCRep)
          .then(() => {
  ...
  if (player.type === 'monster' || player.type === 'npc') {
    ...
    hpInput.value =
      player.type === 'monster'
        ? player.currentHitPoints.toString()
        : player.character.currentHitPoints.toString();
    ...
    hpInput.oninput = () => {
      ...
      if (!isNaN(val)) {
        ...
        NodeCG.waitForReplicants(rep)
          .then(() => {
            if (rep.value) {
              const battleNPC = rep.value[index] as BattleNPC;
              if (battleNPC.type === 'monster') {
                battleNPC.currentHitPoints = val;
              } else {
                battleNPC.character.currentHitPoints = val;
              }
            }
          })
          .catch((err) => {
        ...
        NodeCG.waitForReplicants(rep)
          .then(() => {
            if (rep.value) {
              const battleNPC = rep.value[index] as BattleNPC;
              if (battleNPC.type === 'monster')
                battleNPC.name = nameInput.value;
            }
          })
          .catch((err) => {
            nodecg.log.error(err);
